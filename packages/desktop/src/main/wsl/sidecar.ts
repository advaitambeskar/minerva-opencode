...
export async function spawnWslSidecar(
  ...
  if (!opencode) throw new Error(`Minerva is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export MINERVA_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export MINERVA_CLIENT=desktop",
    `export MINERVA_SERVER_USERNAME=${shellEscape(username)}`,
    `export MINERVA_SERVER_PASSWORD=${shellEscape(password)}`,
