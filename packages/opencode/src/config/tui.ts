...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, any `.agent` or
  // `.opencode` folders between cwd and home, and OPENCODE_CONFIG_DIR.
  ...
  // 4. `.agent` / `.opencode` directories (and OPENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const isProjectDir = (dir: string) => dir.endsWith(".agent") || dir.endsWith(".opencode")
  const dirs = unique(directories).filter((dir) => isProjectDir(dir) || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!isProjectDir(dir) && dir !== Flag.OPENCODE_CONFIG_DIR) continue
