...
import { EventSequenceTable } from "@minerva-ai/core/event/sql"
...
import { WorkspaceTable } from "@minerva-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  MINERVA_AUTH_CONTENT: process.env.MINERVA_AUTH_CONTENT,
  MINERVA_EXPERIMENTAL_WORKSPACES: process.env.MINERVA_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.MINERVA_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.MINERVA_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.MINERVA_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.MINERVA_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.MINERVA_EXPERIMENTAL_WORKSPACES).toBe("true")
