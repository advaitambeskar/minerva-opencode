...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "minerva.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "minerva.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "minerva.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "minerva.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["minerva.json", "minerva.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "minerva.json"),
        path.join(project, "minerva.jsonc"),
      ])
    })
  })
