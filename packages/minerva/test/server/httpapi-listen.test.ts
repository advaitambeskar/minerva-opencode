...
const original = {
  MINERVA_SERVER_PASSWORD: Flag.MINERVA_SERVER_PASSWORD,
  MINERVA_SERVER_USERNAME: Flag.MINERVA_SERVER_USERNAME,
  envPassword: process.env.MINERVA_SERVER_PASSWORD,
  envUsername: process.env.MINERVA_SERVER_USERNAME,
}
const auth = { username: "opencode", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.MINERVA_SERVER_PASSWORD = original.MINERVA_SERVER_PASSWORD
  Flag.MINERVA_SERVER_USERNAME = original.MINERVA_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.MINERVA_SERVER_PASSWORD
  else process.env.MINERVA_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.MINERVA_SERVER_USERNAME
  else process.env.MINERVA_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.MINERVA_SERVER_PASSWORD = auth.password
  Flag.MINERVA_SERVER_USERNAME = auth.username
  process.env.MINERVA_SERVER_PASSWORD = auth.password
  process.env.MINERVA_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.MINERVA_SERVER_PASSWORD = undefined
  Flag.MINERVA_SERVER_USERNAME = auth.username
  delete process.env.MINERVA_SERVER_PASSWORD
  process.env.MINERVA_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
