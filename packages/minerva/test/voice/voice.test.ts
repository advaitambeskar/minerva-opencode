...
describe("Voice", () => {
  it.effect("disabled stub returns off state", () =>
    Effect.gen(function* () {
      const previous = process.env.MINERVA_DISABLE_VOICE
      process.env.MINERVA_DISABLE_VOICE = "true"
      try {
        ...
        if (previous === undefined) delete process.env.MINERVA_DISABLE_VOICE
        else process.env.MINERVA_DISABLE_VOICE = previous
      }
    }),
  )
  ...
  it.effect("transcript buffer submit clears text", () =>
    Effect.gen(function* () {
      const previous = process.env.MINERVA_DISABLE_VOICE
      delete process.env.MINERVA_DISABLE_VOICE
      try {
        const voice = yield* Voice.Service.pipe(Effect.provide(Voice.defaultLayer))
