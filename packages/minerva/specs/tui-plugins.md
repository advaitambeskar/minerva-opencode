...
- Author package entrypoint is `@minerva-ai/plugin/tui`.
...
- Import types from `@minerva-ai/plugin/tui`.
- `@minerva-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@minerva-ai/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `MINERVA_CONFIG_DIR`, OpenCode installs `@minerva-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@minerva-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@minerva-ai/plugin/tui`.
...
- `--pure` / `MINERVA_PURE` skips external TUI plugins only.
