...
- The server still lives in `packages/minerva`.
...
Until enough shared service code lives outside `packages/minerva`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/minerva`

It should not import `packages/minerva` services while `packages/minerva`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/minerva` implementation details.
