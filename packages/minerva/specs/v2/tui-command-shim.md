...
Delete `packages/minerva/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/minerva/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/minerva/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/minerva`
- TUI plugin loader tests in `packages/minerva` if runtime plugin API wiring changed
