...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@minerva-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@minerva-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@minerva-ai/tui/context/theme"
import { Global } from "@minerva-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@minerva-ai/core/util/flock"
import { Flag } from "@minerva-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@minerva-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@minerva-ai/core/v1/config/plugin"
import { createCommandShim } from "@minerva-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@minerva-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".agent"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".agent", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".agent", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.MINERVA_PURE ? [] : pluginOrigins
    if (Flag.MINERVA_PURE && pluginOrigins.length) {
    }
