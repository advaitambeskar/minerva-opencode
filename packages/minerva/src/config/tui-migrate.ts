...
const TUI_SCHEMA_URL = "https://minerva.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from minerva.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const minervaFiles = await configFiles(input)
  for (const file of minervaFiles) {
...
async function configFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "minerva"),
    ...(await Filesystem.findUp(["minerva.json", "minerva.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "minerva"))
  }
  if (Flag.MINERVA_CONFIG) files.push(Flag.MINERVA_CONFIG)
