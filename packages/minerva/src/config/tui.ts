...
import { isRecord } from "@minerva-ai/tui/util/record"
...
import { TuiKeybind } from "@minerva-ai/tui/config/keybind"
...
import { TuiConfig } from "@minerva-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@minerva/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old minerva.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.agent`
  // folders between cwd and home, and MINERVA_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.MINERVA_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit MINERVA_TUI_CONFIG override, if set.
  if (Flag.MINERVA_TUI_CONFIG) {
    const configFile = Flag.MINERVA_TUI_CONFIG
  ...
  // 4. `.agent` directories (and MINERVA_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const isProjectDir = (dir: string) => dir.endsWith(".agent")
  const dirs = unique(directories).filter((dir) => isProjectDir(dir) || dir === Flag.MINERVA_CONFIG_DIR)

  for (const dir of dirs) {
    if (!isProjectDir(dir) && dir !== Flag.MINERVA_CONFIG_DIR) continue
