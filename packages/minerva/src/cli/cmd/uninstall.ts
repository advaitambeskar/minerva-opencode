...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall minerva and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Minerva")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g minerva-ai",
      pnpm: "pnpm uninstall -g minerva-ai",
      bun: "bun remove -g minerva-ai",
      yarn: "yarn global remove minerva-ai",
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string[]> = {
      ...
      bun: ["bun", "remove", "-g", "minerva-ai"],
      yarn: ["yarn", "global", "remove", "minerva-ai"],
      brew: ["brew", "uninstall", "minerva"],
      choco: ["choco", "uninstall", "minerva"],
      scoop: ["scoop", "uninstall", "minerva"],
    }

    const cmd = cmds[method]
    if (cmd) {
      spinner.start(`Running ${cmd.join(" ")}...`)
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "minerva", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Minerva!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# opencode") || content.includes(".minerva/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".minerva/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".minerva/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".minerva"))
    ) {
      continue
    }
