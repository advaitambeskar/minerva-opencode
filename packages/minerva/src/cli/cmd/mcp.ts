...
import { ConfigMCPV1 } from "@minerva-ai/core/v1/config/mcp"
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: minerva mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in minerva.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files. .agent/ is the canonical project dir; .minerva/ is a deprecated fallback.
  const candidates = [path.join(baseDir, "minerva.json"), path.join(baseDir, "minerva.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".agent", "minerva.json"),
      path.join(baseDir, ".agent", "minerva.jsonc"),
      path.join(baseDir, ".minerva", "minerva.json"),
      path.join(baseDir, ".minerva", "minerva.jsonc"),
    )
  }
  ...
  // Default to .agent/minerva.jsonc when creating new
  return global ? candidates[0] : path.join(baseDir, ".agent", "minerva.jsonc")
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., minerva x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "minerva-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
