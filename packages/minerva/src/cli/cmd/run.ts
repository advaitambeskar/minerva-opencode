import type { PermissionV1 } from "@minerva-ai/core/v1/permission"
// CLI entry point for `minerva run`.
//
// Handles three modes:
...
//      minerva server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@minerva-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run minerva with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running minerva server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to MINERVA_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to MINERVA_SERVER_USERNAME or 'minerva')",
      })
      .option("dir", {
        type: "string",
