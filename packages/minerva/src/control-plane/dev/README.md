This is a plugin to simulate a remote environment locally. Add this to `.opencode/minerva.jsonc`:

```json
  "plugin": ["../packages/minerva/src/control-plane/dev/debug-workspace-plugin.ts"],
```
...
```
./packages/minerva/script/run-workspace-server
```
