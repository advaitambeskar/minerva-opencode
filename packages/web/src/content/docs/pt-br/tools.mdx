...
:::note
Esta ferramenta está disponível apenas quando `MINERVA_EXPERIMENTAL_LSP_TOOL=true` (ou `MINERVA_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor opencode ou quando a variável de ambiente `MINERVA_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).
