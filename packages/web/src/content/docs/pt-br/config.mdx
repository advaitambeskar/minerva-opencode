...
2. **Configuração global** (`~/.config/opencode/minerva.json`) - preferências do usuário
3. **Configuração personalizada** (`MINERVA_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`minerva.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.opencode`** - agentes, comandos, plugins
6. **Configuração inline** (`MINERVA_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
Coloque sua configuração global do opencode em `~/.config/opencode/minerva.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Adicione `minerva.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `MINERVA_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `MINERVA_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.opencode`, e deve seguir a mesma estrutura.
...
O esquema de configuração do servidor/tempo de execução é definido em [**`opencode.ai/config.json`**](https://minerva.dev/config.json).
...
Use `MINERVA_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `minerva.json` estão obsoletas e são migradas automaticamente quando possível.
