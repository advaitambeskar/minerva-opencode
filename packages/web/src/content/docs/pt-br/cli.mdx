...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `MINERVA_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `MINERVA_SERVER_USERNAME` ou `opencode`) |
...
Isso inicia um servidor HTTP que fornece acesso à funcionalidade do opencode sem a interface TUI. Defina `MINERVA_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
Isso inicia um servidor HTTP e abre um navegador para acessar o opencode através de uma interface web. Defina `MINERVA_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `MINERVA_AUTO_SHARE`                 | boolean | Compartilhar sessões automaticamente                                  |
| `MINERVA_GIT_BASH_PATH`              | string  | Caminho para o executável do Git Bash no Windows                      |
| `MINERVA_CONFIG`                     | string  | Caminho para o arquivo de configuração                                |
| `MINERVA_CONFIG_DIR`                 | string  | Caminho para o diretório de configuração                              |
| `MINERVA_CONFIG_CONTENT`             | string  | Conteúdo de configuração json inline                                  |
| `MINERVA_DISABLE_AUTOUPDATE`         | boolean | Desabilitar verificações de atualização automática                    |
| `MINERVA_DISABLE_PRUNE`              | boolean | Desabilitar a poda de dados antigos                                   |
| `MINERVA_DISABLE_TERMINAL_TITLE`     | boolean | Desabilitar atualizações automáticas do título do terminal            |
| `MINERVA_PERMISSION`                 | string  | Configuração de permissões json inline                                |
| `MINERVA_DISABLE_DEFAULT_PLUGINS`    | boolean | Desabilitar plugins padrão                                            |
| `MINERVA_DISABLE_LSP_DOWNLOAD`       | boolean | Desabilitar downloads automáticos do servidor LSP                     |
| `MINERVA_ENABLE_EXPERIMENTAL_MODELS` | boolean | Habilitar modelos experimentais                                       |
| `MINERVA_DISABLE_AUTOCOMPACT`        | boolean | Desabilitar compactação automática de contexto                        |
| `MINERVA_DISABLE_CLAUDE_CODE`        | boolean | Desabilitar leitura de `.claude` (prompt + habilidades)               |
| `MINERVA_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Desabilitar leitura de `~/.claude/CLAUDE.md`                          |
| `MINERVA_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Desabilitar carregamento de `.claude/skills`                          |
| `MINERVA_DISABLE_MODELS_FETCH`       | boolean | Desabilitar busca de modelos de fontes remotas                        |
| `MINERVA_FAKE_VCS`                   | string  | Provedor VCS falso para fins de teste                                 |
| `MINERVA_CLIENT`                     | string  | Identificador do cliente (padrão é `cli`)                             |
| `MINERVA_ENABLE_EXA`                 | boolean | Habilitar ferramentas de busca web Exa                                |
| `MINERVA_SERVER_PASSWORD`            | string  | Habilitar autenticação básica para `serve`/`web`                      |
| `MINERVA_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `opencode`) |
| `MINERVA_MODELS_URL`                 | string  | URL personalizada para buscar configuração de modelos                 |
...
| Variável                                        | Tipo    | Descrição                                                 |
| ----------------------------------------------- | ------- | --------------------------------------------------------- |
| `MINERVA_EXPERIMENTAL`                         | boolean | Habilitar recursos experimentais sob o sinalizador global |
| `MINERVA_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Habilitar descoberta de ícones                            |
| `MINERVA_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Desabilitar cópia ao selecionar no TUI                    |
| `MINERVA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Tempo limite padrão para comandos bash em ms              |
| `MINERVA_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Máximo de tokens de saída para respostas LLM              |
| `MINERVA_EXPERIMENTAL_FILEWATCHER`             | boolean | Habilitar monitoramento de arquivos para todo o diretório |
| `MINERVA_EXPERIMENTAL_OXFMT`                   | boolean | Habilitar formatador oxfmt                                |
| `MINERVA_EXPERIMENTAL_LSP_TOOL`                | boolean | Habilitar ferramenta LSP experimental                     |
| `MINERVA_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Desabilitar monitoramento de arquivos                     |
| `MINERVA_EXPERIMENTAL_EXA`                     | boolean | Habilitar recursos experimentais do Exa                   |
| `MINERVA_EXPERIMENTAL_LSP_TY`                  | boolean | Habilitar TY LSP para arquivos python                     |
| `MINERVA_EXPERIMENTAL_PLAN_MODE`               | boolean | Habilitar modo de plano                                   |
| `MINERVA_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Habilitar tarefas de subagentes em segundo plano          |
| `MINERVA_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Habilitar sistema de eventos experimental                 |
| `MINERVA_EXPERIMENTAL_NATIVE_LLM`              | boolean | Habilitar caminho nativo de requisição LLM                |
| `MINERVA_EXPERIMENTAL_PARALLEL`                | boolean | Habilitar execução paralela de busca web                  |
| `MINERVA_EXPERIMENTAL_SCOUT`                   | boolean | Habilitar subagente Scout                                 |
| `MINERVA_EXPERIMENTAL_WORKSPACES`              | boolean | Habilitar suporte a espaços de trabalho                   |
