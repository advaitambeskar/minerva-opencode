...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/opencode/minerva.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\minerva.jsonc`
...
Se seu `minerva.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `MINERVA_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `MINERVA_PORT` (ou escolha uma porta livre) e reinicie.
