...
- **macOS/Linux**: `~/.config/opencode/minerva.jsonc`（或 `~/.config/opencode/minerva.json`）
- **macOS/Linux**（舊版安裝）: `~/.local/share/opencode/minerva.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\opencode\minerva.jsonc`
...
如果你的 `minerva.json(c)` 包含 `server` 部分，請暫時移除該部分並重新啟動桌面應用程式。

#### 檢查環境變數

如果你在環境中設定了 `MINERVA_PORT`，桌面應用程式將嘗試使用該連接埠作為本地伺服器連接埠。

- 取消設定 `MINERVA_PORT`（或選擇一個空閒連接埠）並重新啟動。
