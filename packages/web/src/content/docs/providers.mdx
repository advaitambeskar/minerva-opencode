...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `minerva.json`:
...
```json title="minerva.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://minerva.dev/config.json",
  "provider": {
    "atomic-chat": {
...
`minerva.json` file. It is also recommended to disable session sharing.
...
Customize through `minerva.json`:
...
```jsonc title="~/.config/opencode/minerva.jsonc"
{
  "$schema": "https://minerva.dev/config.json",
  "provider": {
    "helicone": {
...
```json title="minerva.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://minerva.dev/config.json",
  "provider": {
    "llama.cpp": {
...
```json title="minerva.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://minerva.dev/config.json",
  "provider": {
    "lmstudio": {
...
```json title="minerva.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://minerva.dev/config.json",
  "provider": {
    "ollama": {
...
The model catalog is provided automatically. A minimal `minerva.json` is all that's needed:
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in minerva.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `minerva.json` file in your project directory:

   ```json title="minerva.json" ""myprovider"" {5-15}
   {
     "$schema": "https://minerva.dev/config.json",
     "provider": {
       "myprovider": {
...
```json title="minerva.json" {9,11,17-20}
{
  "$schema": "https://minerva.dev/config.json",
  "provider": {
    "myprovider": {
