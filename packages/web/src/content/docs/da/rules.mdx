...
Du kan spesifisere egendefinerte instruksjonsfiler i din `minerva.json` eller den globale `~/.config/opencode/minerva.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
### Brug af minerva.json

Den anbefalte tilnærmingen er at bruge feltet `instructions` i `minerva.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er brug av `minerva.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
