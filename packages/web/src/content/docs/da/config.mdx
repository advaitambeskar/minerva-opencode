...
2. **Global config** (`~/.config/opencode/minerva.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`MINERVA_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`minerva.json` i projekt) - projektspecifikke indstillinger
5. **`.opencode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`MINERVA_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
Placer din globale OpenCode-konfiguration i `~/.config/opencode/minerva.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `minerva.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `MINERVA_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `MINERVA_CONFIG_DIR`
...
Konfigurationsfilen har et skema, der defineres i [**`opencode.ai/config.json`**](https://minerva.dev/config.json).
