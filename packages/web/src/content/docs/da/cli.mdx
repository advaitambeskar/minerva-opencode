...
| <nobr><code>{"--session"}</code></nobr>  | `-s` | Sessions-id for at fortsætte                                                      |
| <nobr><code>{"--fork"}</code></nobr>     |      | Forgren sessionen ved fortsættelse (brug med `--continue` eller `--session`)      |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `MINERVA_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `MINERVA_SERVER_USERNAME` eller `opencode`) |
...
Dette starter en HTTP-server, der giver API-adgang til opencode-funktionalitet uden TUI-grænsefladen. Indstil `MINERVA_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenCode via en webgrænseflade. Indstil `MINERVA_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `MINERVA_AUTO_SHARE`                 | boolean | Del automatisk session                                                |
| `MINERVA_GIT_BASH_PATH`              | string  | Sti til Git Bash eksekverbar på Windows                               |
| `MINERVA_CONFIG`                     | string  | Sti til konfigurationsfil                                             |
| `MINERVA_TUI_CONFIG`                 | string  | Sti til TUI-konfigurationsfil                                         |
| `MINERVA_CONFIG_DIR`                 | string  | Sti til konfigurationsmappe                                           |
| `MINERVA_CONFIG_CONTENT`             | string  | Indbygget json-konfigurationsindhold                                  |
| `MINERVA_DISABLE_AUTOUPDATE`         | boolean | Deaktiver automatisk opdateringskontrol                               |
| `MINERVA_DISABLE_PRUNE`              | boolean | Deaktiver beskæring af gamle data                                     |
| `MINERVA_DISABLE_TERMINAL_TITLE`     | boolean | Deaktiver automatisk opdatering af terminaltitel                      |
| `MINERVA_PERMISSION`                 | string  | Indbygget json-tilladelseskonfiguration                               |
| `MINERVA_DISABLE_DEFAULT_PLUGINS`    | boolean | Deaktiver standard plugins                                            |
| `MINERVA_DISABLE_LSP_DOWNLOAD`       | boolean | Deaktiver automatisk LSP-serverdownloads                              |
| `MINERVA_ENABLE_EXPERIMENTAL_MODELS` | boolean | Aktive eksperimentelle modeller                                       |
| `MINERVA_DISABLE_AUTOCOMPACT`        | boolean | Deaktiver automatisk kontekstkomprimering                             |
| `MINERVA_DISABLE_CLAUDE_CODE`        | boolean | Deaktiver læsning fra `.claude` (prompt + færdigheder)                |
| `MINERVA_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Deaktiver læsning `~/.claude/CLAUDE.md`                               |
| `MINERVA_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Deaktiver indlæsning af `.claude/skills`                              |
| `MINERVA_DISABLE_MODELS_FETCH`       | boolean | Deaktivering af modeller fra eksterne kilder                          |
| `MINERVA_FAKE_VCS`                   | string  | Falsk VCS-udbyder til testformål                                      |
| `MINERVA_CLIENT`                     | string  | Klient-id (standard til `cli`)                                        |
| `MINERVA_ENABLE_EXA`                 | boolean | Aktiver Exa-websøgeværktøjer                                          |
| `MINERVA_SERVER_PASSWORD`            | string  | Aktiver grundlæggende godkendelse for `serve`/`web`                   |
| `MINERVA_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `opencode`) |
| `MINERVA_MODELS_URL`                 | string  | Brugerdefineret URL til hentning af modelkonfiguration                |
...
| Variabel                                        | Skriv   | Beskrivelse                                          |
| ----------------------------------------------- | ------- | ---------------------------------------------------- |
| `MINERVA_EXPERIMENTAL`                         | boolean | Aktiver eksperimentelle funktioner under samleflaget |
| `MINERVA_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Aktiver ikonopdagelse                                |
| `MINERVA_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Deaktiver kopi ved valg i TUI                        |
| `MINERVA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | nummer  | Standard timeout for bash-kommandoer i ms            |
| `MINERVA_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | nummer  | Maks. output-tokens for LLM-svar                     |
| `MINERVA_EXPERIMENTAL_FILEWATCHER`             | boolean | Aktiver filovervågning for hele dir                  |
| `MINERVA_EXPERIMENTAL_OXFMT`                   | boolean | Aktiver oxfmt formatter                              |
| `MINERVA_EXPERIMENTAL_LSP_TOOL`                | boolean | Aktive eksperimenter LSP værktøj                     |
| `MINERVA_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Deaktiver filovervågning                             |
| `MINERVA_EXPERIMENTAL_EXA`                     | boolean | Aktive eksperimenter Exa-funktioner                  |
| `MINERVA_EXPERIMENTAL_LSP_TY`                  | boolean | Aktiver TY LSP for python-filer                      |
| `MINERVA_EXPERIMENTAL_PLAN_MODE`               | boolean | Aktiver plantilstand                                 |
| `MINERVA_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Aktiver baggrundsopgaver for subagenter              |
| `MINERVA_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Aktiver eksperimentelt hændelsessystem               |
| `MINERVA_EXPERIMENTAL_NATIVE_LLM`              | boolean | Aktiver native LLM-anmodningssti                     |
| `MINERVA_EXPERIMENTAL_PARALLEL`                | boolean | Aktiver parallel udførelse af websøgning             |
| `MINERVA_EXPERIMENTAL_SCOUT`                   | boolean | Aktiver Scout-subagent                               |
| `MINERVA_EXPERIMENTAL_WORKSPACES`              | boolean | Aktiver workspace-understøttelse                     |
