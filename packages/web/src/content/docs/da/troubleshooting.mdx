...
- **macOS/Linux**: `~/.config/opencode/minerva.jsonc` (eller `~/.config/opencode/minerva.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/opencode/minerva.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\minerva.jsonc`
...
Hvis `minerva.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `MINERVA_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `MINERVA_PORT` (eller vælg en ledig port) og start på ny.
