...
- **macOS/Linux**: `~/.config/opencode/minerva.jsonc` (or `~/.config/opencode/minerva.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/minerva.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\minerva.jsonc`
...
If your `minerva.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `MINERVA_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `MINERVA_PORT` (or pick a free port) and restart.
