...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `minerva.json`:
...
`minerva.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
4. Utwórz lub zaktualizuj plik `minerva.json` w katalogu projektu:
