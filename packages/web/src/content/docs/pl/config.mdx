...
2. **Konfiguracja globalna** (`~/.config/opencode/minerva.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `MINERVA_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`minerva.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.opencode`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `MINERVA_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
Umieść swoją globalną konfigurację OpenCode w `~/.config/opencode/minerva.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.
...
Dodaj `minerva.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `MINERVA_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `MINERVA_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.opencode` i powinien zachować tę samą strukturę.
...
Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`opencode.ai/config.json`**](https://minerva.dev/config.json).
...
Użyj `MINERVA_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `minerva.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
