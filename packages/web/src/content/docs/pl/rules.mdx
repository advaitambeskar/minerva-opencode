...
Możesz określić niestandardowe pliki instrukcji w swoim `minerva.json` lub globalnym `~/.config/opencode/minerva.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
### Używanie minerva.json

Zalecanym podejściem jest użycie pola `instructions` w `minerva.json`:
...
:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `minerva.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
