...
- **macOS/Linux**: `~/.config/opencode/minerva.jsonc` (lub `~/.config/opencode/minerva.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencode/minerva.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\minerva.jsonc`
...
Jeśli Twój `minerva.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `MINERVA_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `MINERVA_PORT` (lub wybierz wolny port) i uruchom ponownie.
