...
2. **Global config** (`~/.config/opencode/minerva.json`) - user preferences
3. **Custom config** (`MINERVA_CONFIG` env var) - custom overrides
4. **Project config** (`minerva.json` in project) - project-specific settings
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config** (`MINERVA_CONFIG_CONTENT` env var) - runtime overrides
...
Place your global OpenCode config in `~/.config/opencode/minerva.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
Add `minerva.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
Specify a custom config file path using the `MINERVA_CONFIG` environment variable.
...
Specify a custom config directory using the `MINERVA_CONFIG_DIR`
...
Drop an `minerva.json` or `minerva.jsonc` file in the system managed config directory:
...
The plist keys map directly to `minerva.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.
...
The server/runtime config schema is defined in [**`opencode.ai/config.json`**](https://minerva.dev/config.json).
...
Use `MINERVA_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `minerva.json` are deprecated and automatically migrated when possible.
