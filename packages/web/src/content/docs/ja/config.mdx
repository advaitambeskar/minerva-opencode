...
2. **グローバル設定** (`~/.config/opencode/minerva.json`) - ユーザー設定
3. **カスタム設定** (`MINERVA_CONFIG` 環境変数) - カスタムオーバーライド
4. **プロジェクト設定** (プロジェクト内の `minerva.json`) - プロジェクト固有の設定
5. **`.opencode` ディレクトリ** - エージェント、コマンド、プラグイン
6. **インライン設定** (`MINERVA_CONFIG_CONTENT` 環境変数) - ランタイムオーバーライド
...
グローバル OpenCode 設定を `~/.config/opencode/minerva.json` に配置します。プロバイダー、モデル、権限などのユーザー全体の設定にはグローバル設定を使用します。
...
プロジェクトのルートに `minerva.json` を追加します。プロジェクト設定は、標準設定ファイルの中で最も高い優先順位を持ち、グローバル設定とリモート設定の両方をオーバーライドします。
...
`MINERVA_CONFIG` 環境変数を使用してカスタム設定ファイルのパスを指定します。
...
`MINERVA_CONFIG_DIR` を使用してカスタム設定ディレクトリを指定します。
...
サーバー/ランタイム構成スキーマは [**`opencode.ai/config.json`**](https://minerva.dev/config.json) で定義されています。
...
カスタム TUI 設定ファイルを指定するには、`MINERVA_TUI_CONFIG` を使用します。

`minerva.json` 内の従来の `theme`、`keybinds`、および `tui` キーは非推奨となり、可能であれば自動的に移行されます。
