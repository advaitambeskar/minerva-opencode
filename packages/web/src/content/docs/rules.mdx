...
You can specify custom instruction files in your `minerva.json` or the global `~/.config/opencode/minerva.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
The recommended approach is to use the `instructions` field in `minerva.json`:
...
:::tip
For monorepos or projects with shared standards, using `minerva.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::
