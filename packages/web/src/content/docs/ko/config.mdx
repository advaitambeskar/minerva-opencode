...
2. **Global config**(`~/.config/opencode/minerva.json`) - 사용자 기본 설정
3. **Custom config**(`MINERVA_CONFIG` env var) - custom override
4. **Project config**(프로젝트의 `minerva.json`) - 프로젝트별 설정
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config**(`MINERVA_CONFIG_CONTENT` env var) - 런타임 override
...
전역 OpenCode config는 `~/.config/opencode/minerva.json`에 두세요. provider, model, permissions 같은 사용자 전체 기본 설정은 전역 config로 관리하세요.
...
프로젝트 루트에 `minerva.json`을 추가하세요. 프로젝트 config는 표준 config 파일 중 우선순위가 가장 높아 전역 및 Remote config를 모두 override합니다.
...
`MINERVA_CONFIG` 환경 변수로 custom config 파일 경로를 지정하세요.
...
`MINERVA_CONFIG_DIR` 환경 변수로 custom config 디렉토리를 지정할 수 있습니다. 이 디렉토리는 표준 `.opencode` 디렉토리와 동일하게 agents, commands, modes, plugins를 검색하며, 동일한 구조를 따라야 합니다.
...
server/runtime config schema는 [**`opencode.ai/config.json`**](https://minerva.dev/config.json)에 정의되어 있습니다.
...
`MINERVA_TUI_CONFIG`를 사용하여 사용자 지정 TUI 설정 파일을 가리킬 수 있습니다.

`minerva.json`의 기존 `theme`, `keybinds`, `tui` 키는 더 이상 사용되지 않으며(deprecated) 가능한 경우 자동으로 마이그레이션됩니다.
