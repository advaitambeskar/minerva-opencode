...
:::note
Ovaj alat je dostupan samo kada je `MINERVA_EXPERIMENTAL_LSP_TOOL=true` (ili `MINERVA_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `MINERVA_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).
