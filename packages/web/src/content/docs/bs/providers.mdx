...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `minerva.json`:
...
`minerva.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `minerva.json`:
...
4. Kreirajte ili ažurirajte svoju `minerva.json` datoteku u direktoriju projekta:
