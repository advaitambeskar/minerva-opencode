...
Mozete navesti prilagodene datoteke uputstava u `minerva.json` ili globalnom `~/.config/opencode/minerva.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
### Korištenje minerva.json

Preporuceni pristup je da koristite `instructions` polje u `minerva.json`:
...
:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `minerva.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
