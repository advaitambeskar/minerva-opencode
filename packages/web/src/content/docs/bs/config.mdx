...
2. **Globalna konfiguracija** (`~/.config/opencode/minerva.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`MINERVA_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`minerva.json` u projektu) - postavke specifične za projekat
5. **`.opencode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`MINERVA_CONFIG_CONTENT` env var) - runtime preinačenja
...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/opencode/minerva.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Dodajte `minerva.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `MINERVA_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `MINERVA_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.opencode` direktorij, i trebali bi pratiti istu strukturu.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`opencode.ai/config.json`**](https://minerva.dev/config.json).
...
Koristite `MINERVA_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `minerva.json` su zastarjeli i automatski će se migrirati kada je to moguće.
