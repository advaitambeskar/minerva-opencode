...
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `MINERVA_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `MINERVA_SERVER_USERNAME` ili `opencode`) |
...
Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti OpenCode-a bez TUI interfejsa. Postavite `MINERVA_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenCode-u preko web interfejsa. Postavite `MINERVA_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
| Varijabla                             | Tip     | Opis                                                               |
| ------------------------------------- | ------- | ------------------------------------------------------------------ |
| `MINERVA_AUTO_SHARE`                 | boolean | Automatski dijeli sesije                                           |
| `MINERVA_GIT_BASH_PATH`              | string  | Putanja do Git Bash izvršne datoteke na Windows-u                  |
| `MINERVA_CONFIG`                     | string  | Putanja do konfiguracijskog fajla                                  |
| `MINERVA_TUI_CONFIG`                 | string  | Putanja do TUI konfiguracijskog fajla                              |
| `MINERVA_CONFIG_DIR`                 | string  | Putanja do konfiguracijskog direktorija                            |
| `MINERVA_CONFIG_CONTENT`             | string  | Inline json konfiguracijski sadržaj                                |
| `MINERVA_DISABLE_AUTOUPDATE`         | boolean | Onemogući automatske provjere ažuriranja                           |
| `MINERVA_DISABLE_PRUNE`              | boolean | Onemogući brisanje (pruning) starih podataka                       |
| `MINERVA_DISABLE_TERMINAL_TITLE`     | boolean | Onemogući automatsko ažuriranje naslova terminala                  |
| `MINERVA_PERMISSION`                 | string  | Inline json konfiguracija dozvola                                  |
| `MINERVA_DISABLE_DEFAULT_PLUGINS`    | boolean | Onemogući podrazumijevane dodatke (plugins)                        |
| `MINERVA_DISABLE_LSP_DOWNLOAD`       | boolean | Onemogući automatsko preuzimanje LSP servera                       |
| `MINERVA_ENABLE_EXPERIMENTAL_MODELS` | boolean | Omogući eksperimentalne modele                                     |
| `MINERVA_DISABLE_AUTOCOMPACT`        | boolean | Onemogući automatsko sažimanje konteksta                           |
| `MINERVA_DISABLE_CLAUDE_CODE`        | boolean | Onemogući čitanje iz `.claude` (prompt + vještine)                 |
| `MINERVA_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Onemogući čitanje `~/.claude/CLAUDE.md`                            |
| `MINERVA_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Onemogući učitavanje `.claude/skills`                              |
| `MINERVA_DISABLE_MODELS_FETCH`       | boolean | Onemogući dohvaćanje modela iz udaljenih izvora                    |
| `MINERVA_FAKE_VCS`                   | string  | Lažni VCS provajder za potrebe testiranja                          |
| `MINERVA_CLIENT`                     | string  | Identifikator klijenta (zadano na `cli`)                           |
| `MINERVA_ENABLE_EXA`                 | boolean | Omogući Exa alate za web pretraživanje                             |
| `MINERVA_SERVER_PASSWORD`            | string  | Omogući osnovnu autentifikaciju za `serve`/`web`                   |
| `MINERVA_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `opencode`) |
| `MINERVA_MODELS_URL`                 | string  | Prilagođeni URL za dohvaćanje konfiguracije modela                 |
...
| Varijabla                                       | Tip     | Opis                                                    |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `MINERVA_EXPERIMENTAL`                         | boolean | Omogući eksperimentalne funkcije pod zbirnom zastavicom |
| `MINERVA_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Omogući otkrivanje ikona                                |
| `MINERVA_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Onemogući kopiranje pri odabiru u TUI                   |
| `MINERVA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Zadano vremensko ograničenje za bash naredbe u ms       |
| `MINERVA_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Maksimalni izlazni tokeni za LLM odgovore               |
| `MINERVA_EXPERIMENTAL_FILEWATCHER`             | boolean | Omogući praćenje datoteka za cijeli direktorij          |
| `MINERVA_EXPERIMENTAL_OXFMT`                   | boolean | Omogući oxfmt formatter                                 |
| `MINERVA_EXPERIMENTAL_LSP_TOOL`                | boolean | Omogući eksperimentalni LSP alat                        |
| `MINERVA_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Onemogući praćenje datoteka                             |
| `MINERVA_EXPERIMENTAL_EXA`                     | boolean | Omogući eksperimentalne Exa funkcije                    |
| `MINERVA_EXPERIMENTAL_LSP_TY`                  | boolean | Omogući TY LSP za python datoteke                       |
| `MINERVA_EXPERIMENTAL_PLAN_MODE`               | boolean | Omogući Plan mod                                        |
| `MINERVA_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Omogući pozadinske zadatke subagenata                   |
| `MINERVA_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Omogući eksperimentalni sistem događaja                 |
| `MINERVA_EXPERIMENTAL_NATIVE_LLM`              | boolean | Omogući nativnu putanju LLM zahtjeva                    |
| `MINERVA_EXPERIMENTAL_PARALLEL`                | boolean | Omogući paralelno izvršavanje web pretrage              |
| `MINERVA_EXPERIMENTAL_SCOUT`                   | boolean | Omogući Scout subagenta                                 |
| `MINERVA_EXPERIMENTAL_WORKSPACES`              | boolean | Omogući podršku za radne prostore                       |
