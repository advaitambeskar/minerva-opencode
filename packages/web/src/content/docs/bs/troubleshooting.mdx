...
- **macOS/Linux**: `~/.config/opencode/minerva.jsonc` (ili `~/.config/opencode/minerva.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opencode/minerva.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\minerva.jsonc`
...
Ako vaš `minerva.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `MINERVA_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `MINERVA_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
