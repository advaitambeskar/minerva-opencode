...
2. **Globale Konfiguration** (`~/.config/opencode/minerva.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`MINERVA_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`minerva.json` im Projekt) – projektspezifische Einstellungen
5. **`.opencode` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`MINERVA_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
Platzieren Sie Ihre globale OpenCode-Konfiguration in `~/.config/opencode/minerva.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `minerva.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.
...
Geben Sie mithilfe der Umgebungsvariablen `MINERVA_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `MINERVA_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
...
Die Konfigurationsdatei verfügt über ein Schema, das in [**`opencode.ai/config.json`**](https://minerva.dev/config.json) definiert ist.
