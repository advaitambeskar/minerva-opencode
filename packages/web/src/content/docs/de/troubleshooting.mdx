...
- **macOS/Linux**: `~/.config/opencode/minerva.jsonc` (oder `~/.config/opencode/minerva.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opencode/minerva.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\minerva.jsonc`
...
Wenn deine `minerva.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `MINERVA_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `MINERVA_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
1. **Probleme auf GitHub melden**
   ...
   [**github.com/advaitambeskar/minerva-opencode/issues**](https://github.com/advaitambeskar/minerva-opencode/issues)
