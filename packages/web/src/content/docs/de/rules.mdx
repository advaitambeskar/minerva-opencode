...
Du kannst zusaetzliche Anweisungsdateien in deiner `minerva.json` oder in `~/.config/opencode/minerva.json` angeben.
...
Empfohlen ist das `instructions`-Feld in der `minerva.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `minerva.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
