...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `minerva.json`:
...
`minerva.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
4. Erstellen oder aktualisieren Sie Ihre `minerva.json`-Datei in Ihrem Projektverzeichnis:
