...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/opencode/minerva.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\minerva.jsonc`
...
Se il tuo `minerva.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `MINERVA_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `MINERVA_PORT` (o scegli una porta libera) e riavvia.
