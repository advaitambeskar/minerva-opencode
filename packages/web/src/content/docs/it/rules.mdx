...
Puoi specificare file di istruzioni personalizzati nel tuo `minerva.json` o nel globale `~/.config/opencode/minerva.json`. Questo permette a te e al tuo team di riusare regole esistenti invece di doverle duplicare in AGENTS.md.
...
L'approccio consigliato e' usare il campo `instructions` in `minerva.json`:
...
:::tip
Per monorepo o progetti con standard condivisi, usare `minerva.json` con pattern glob (come `packages/*/AGENTS.md`) e' piu' manutenibile rispetto alle istruzioni manuali.
:::
