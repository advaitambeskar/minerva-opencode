...
:::caution
Se `MINERVA_SERVER_PASSWORD` non e impostata, il server non sara protetto. Va bene per l'uso locale, ma va impostata per l'accesso in rete.
:::
...
Per proteggere l'accesso, imposta una password usando la variabile d'ambiente `MINERVA_SERVER_PASSWORD`:

```bash
MINERVA_SERVER_PASSWORD=secret opencode web
```

Lo username predefinito e `opencode`, ma puo essere cambiato con `MINERVA_SERVER_USERNAME`.
...
Puoi anche configurare le impostazioni del server nel file di config `minerva.json`:
