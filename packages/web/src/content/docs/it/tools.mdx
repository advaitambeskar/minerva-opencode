...
:::note
Questo strumento e' disponibile solo quando `MINERVA_EXPERIMENTAL_LSP_TOOL=true` (oppure `MINERVA_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider OpenCode o quando la variabile d'ambiente `MINERVA_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).
