...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `MINERVA_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `MINERVA_SERVER_USERNAME` o `opencode`) |
...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `MINERVA_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `MINERVA_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `MINERVA_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `MINERVA_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `MINERVA_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `MINERVA_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `MINERVA_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `MINERVA_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `MINERVA_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `MINERVA_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `MINERVA_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `MINERVA_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `MINERVA_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `MINERVA_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `MINERVA_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `MINERVA_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `MINERVA_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `MINERVA_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `MINERVA_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `MINERVA_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `MINERVA_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `MINERVA_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `MINERVA_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `MINERVA_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `MINERVA_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `opencode`)     |
| `MINERVA_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                             |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `MINERVA_EXPERIMENTAL`                         | boolean | Abilita funzionalità sperimentali sotto il flag globale |
| `MINERVA_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                                  |
| `MINERVA_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI                     |
| `MINERVA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms               |
| `MINERVA_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM                |
| `MINERVA_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir                   |
| `MINERVA_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                                 |
| `MINERVA_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale                      |
| `MINERVA_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                                 |
| `MINERVA_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalità Exa sperimentali                   |
| `MINERVA_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python                        |
| `MINERVA_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                                       |
| `MINERVA_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Abilita task subagent in background                     |
| `MINERVA_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Abilita sistema eventi sperimentale                     |
| `MINERVA_EXPERIMENTAL_NATIVE_LLM`              | boolean | Abilita percorso nativo di richiesta LLM                |
| `MINERVA_EXPERIMENTAL_PARALLEL`                | boolean | Abilita esecuzione parallela della ricerca web          |
| `MINERVA_EXPERIMENTAL_SCOUT`                   | boolean | Abilita subagent Scout                                  |
| `MINERVA_EXPERIMENTAL_WORKSPACES`              | boolean | Abilita supporto workspace                              |
