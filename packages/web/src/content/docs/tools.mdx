...
:::note
This tool is only available when `MINERVA_EXPERIMENTAL_LSP_TOOL=true` (or `MINERVA_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the OpenCode provider or when the `MINERVA_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).
