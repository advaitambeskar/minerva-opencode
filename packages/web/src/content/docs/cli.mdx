...
| <nobr><code>{"--session"}</code></nobr>  | `-s`  | Session ID to continue                                                     |
| <nobr><code>{"--fork"}</code></nobr>     |       | Fork the session when continuing (use with `--continue` or `--session`)    |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `MINERVA_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `MINERVA_SERVER_USERNAME` or `opencode`) |
...
This starts an HTTP server that provides API access to opencode functionality without the TUI interface. Set `MINERVA_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
This starts an HTTP server and opens a web browser to access OpenCode through a web interface. Set `MINERVA_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
| ------------------------------------- | ------- | ------------------------------------------------- |
| `MINERVA_AUTO_SHARE`                 | boolean | Automatically share sessions                      |
| `MINERVA_GIT_BASH_PATH`              | string  | Path to Git Bash executable on Windows            |
| `MINERVA_CONFIG`                     | string  | Path to config file                               |
| `MINERVA_TUI_CONFIG`                 | string  | Path to TUI config file                           |
| `MINERVA_CONFIG_DIR`                 | string  | Path to config directory                          |
| `MINERVA_CONFIG_CONTENT`             | string  | Inline json config content                        |
| `MINERVA_DISABLE_AUTOUPDATE`         | boolean | Disable automatic update checks                   |
| `MINERVA_DISABLE_PRUNE`              | boolean | Disable pruning of old data                       |
| `MINERVA_DISABLE_TERMINAL_TITLE`     | boolean | Disable automatic terminal title updates          |
| `MINERVA_PERMISSION`                 | string  | Inlined json permissions config                   |
| `MINERVA_DISABLE_DEFAULT_PLUGINS`    | boolean | Disable default plugins                           |
| `MINERVA_DISABLE_LSP_DOWNLOAD`       | boolean | Disable automatic LSP server downloads            |
| `MINERVA_ENABLE_EXPERIMENTAL_MODELS` | boolean | Enable experimental models                        |
| `MINERVA_DISABLE_AUTOCOMPACT`        | boolean | Disable automatic context compaction              |
| `MINERVA_DISABLE_CLAUDE_CODE`        | boolean | Disable reading from `.claude` (prompt + skills)  |
| `MINERVA_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disable reading `~/.claude/CLAUDE.md`             |
| `MINERVA_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disable loading `.claude/skills`                  |
| `MINERVA_DISABLE_MODELS_FETCH`       | boolean | Disable fetching models from remote sources       |
| `MINERVA_DISABLE_MOUSE`              | boolean | Disable mouse capture in the TUI                  |
| `MINERVA_FAKE_VCS`                   | string  | Fake VCS provider for testing purposes            |
| `MINERVA_CLIENT`                     | string  | Client identifier (defaults to `cli`)             |
| `MINERVA_ENABLE_EXA`                 | boolean | Enable Exa web search tools                       |
| `MINERVA_SERVER_PASSWORD`            | string  | Enable basic auth for `serve`/`web`               |
| `MINERVA_SERVER_USERNAME`            | string  | Override basic auth username (default `opencode`) |
| `MINERVA_MODELS_URL`                 | string  | Custom URL for fetching models configuration      |
...
| Variable                                        | Type    | Description                             |
| ----------------------------------------------- | ------- | --------------------------------------- |
| `MINERVA_EXPERIMENTAL`                         | boolean | Enable the experimental umbrella flag   |
| `MINERVA_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Enable icon discovery                   |
| `MINERVA_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disable copy on select in TUI           |
| `MINERVA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Default timeout for bash commands in ms |
| `MINERVA_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Max output tokens for LLM responses     |
| `MINERVA_EXPERIMENTAL_FILEWATCHER`             | boolean | Enable file watcher for entire dir      |
| `MINERVA_EXPERIMENTAL_OXFMT`                   | boolean | Enable oxfmt formatter                  |
| `MINERVA_EXPERIMENTAL_LSP_TOOL`                | boolean | Enable experimental LSP tool            |
| `MINERVA_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disable file watcher                    |
| `MINERVA_EXPERIMENTAL_EXA`                     | boolean | Enable experimental Exa features        |
| `MINERVA_EXPERIMENTAL_LSP_TY`                  | boolean | Enable TY LSP for python files          |
| `MINERVA_EXPERIMENTAL_PLAN_MODE`               | boolean | Enable plan mode                        |
| `MINERVA_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Enable background subagent tasks        |
| `MINERVA_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Enable experimental event system        |
| `MINERVA_EXPERIMENTAL_NATIVE_LLM`              | boolean | Enable native LLM request path          |
| `MINERVA_EXPERIMENTAL_PARALLEL`                | boolean | Enable parallel web search execution    |
| `MINERVA_EXPERIMENTAL_SCOUT`                   | boolean | Enable Scout subagent                   |
| `MINERVA_EXPERIMENTAL_WORKSPACES`              | boolean | Enable workspace support                |
