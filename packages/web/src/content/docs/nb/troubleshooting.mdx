...
- **macOS/Linux** (eldre installasjoner): `~/.local/share/opencode/minerva.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\minerva.jsonc`
...
Hvis `minerva.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.

#### Sjekk miljøvariabler

Hvis du har `MINERVA_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `MINERVA_PORT` (eller velg en ledig port) og start på nytt.
