...
Du kan spesifisere egendefinerte instruksjonsfiler i din `minerva.json` eller den globale `~/.config/opencode/minerva.json`. Dette lar deg og teamet ditt gjenbruke eksisterende regler i stedet for å måtte duplisere dem til AGENTS.md.
...
Den anbefalte tilnærmingen er å bruke feltet `instructions` i `minerva.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er bruk av `minerva.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
