...
For å beskytte tilgang, angi et passord ved hjelp av miljøvariabelen `MINERVA_SERVER_PASSWORD`:

```bash
MINERVA_SERVER_PASSWORD=secret opencode web
```

Brukernavnet er satt til `opencode` som standard, men kan endres med `MINERVA_SERVER_USERNAME`.
...
Du kan også konfigurere serverinnstillinger i `minerva.json` konfigurasjonsfilen:
