...
:::note
Dette verktøyet er bare tilgjengelig når `MINERVA_EXPERIMENTAL_LSP_TOOL=true` (eller `MINERVA_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenCode-leverandøren eller når miljøvariabelen `MINERVA_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).
