...
2. **Global konfigurasjon** (`~/.config/opencode/minerva.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`MINERVA_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`minerva.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.opencode` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`MINERVA_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/opencode/minerva.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `minerva.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `MINERVA_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `MINERVA_CONFIG_DIR`
...
Konfigurasjonsfilen har et skjema som er definert i [**`opencode.ai/config.json`**](https://minerva.dev/config.json).
