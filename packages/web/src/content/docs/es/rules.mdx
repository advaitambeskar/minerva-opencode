...
Puede especificar archivos de instrucciones personalizados en su `minerva.json` o el `~/.config/opencode/minerva.json` global. Esto le permite a usted y a su equipo reutilizar las reglas existentes en lugar de tener que duplicarlas en AGENTS.md.
...
El enfoque recomendado es utilizar el campo `instructions` en `minerva.json`:
...
:::tip
Para monorepos o proyectos con estándares compartidos, usar `minerva.json` con patrones globales (como `packages/*/AGENTS.md`) es más fácil de mantener que las instrucciones manuales.
:::
