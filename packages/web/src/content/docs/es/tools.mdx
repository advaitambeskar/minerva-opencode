...
:::note
Esta herramienta solo está disponible cuando `MINERVA_EXPERIMENTAL_LSP_TOOL=true` (o `MINERVA_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenCode o cuando la variable de entorno `MINERVA_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenCode:

```bash
MINERVA_ENABLE_EXA=1 opencode
```
