...
- **macOS/Linux**: `~/.config/opencode/minerva.jsonc` (o `~/.config/opencode/minerva.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/opencode/minerva.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opencode\minerva.jsonc`
...
Si su `minerva.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `MINERVA_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `MINERVA_PORT` (o elija un puerto libre) y reinicie.
