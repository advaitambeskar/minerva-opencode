...
:::caution
Si no se establece `MINERVA_SERVER_PASSWORD`, el servidor no será seguro. Esto está bien para uso local, pero debe configurarse para acceso a la red.
:::
...
Para proteger el acceso, establezca una contraseña utilizando la variable de entorno `MINERVA_SERVER_PASSWORD`:

```bash
MINERVA_SERVER_PASSWORD=secret opencode web
```

El nombre de usuario predeterminado es `opencode` pero se puede cambiar con `MINERVA_SERVER_USERNAME`.
...
También puede configurar los ajustes del servidor en su archivo de configuración `minerva.json`:
