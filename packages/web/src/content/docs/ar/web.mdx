...
:::caution
إذا لم يتم تعيين `MINERVA_SERVER_PASSWORD` فسيكون الخادم دون حماية. هذا مناسب للاستخدام المحلي، لكنه يجب أن يكون مُعينا عند إتاحة الوصول عبر الشبكة.
:::
...
لحماية الوصول، عيّن كلمة مرور عبر متغير البيئة `MINERVA_SERVER_PASSWORD`:

```bash
MINERVA_SERVER_PASSWORD=secret opencode web
```

اسم المستخدم الافتراضي هو `opencode`، ويمكن تغييره عبر `MINERVA_SERVER_USERNAME`.
...
يمكنك أيضا ضبط إعدادات الخادم داخل ملف الإعدادات `minerva.json`:
