...
:::note
هذه الأداة متاحة فقط عند ضبط `MINERVA_EXPERIMENTAL_LSP_TOOL=true` (أو `MINERVA_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `MINERVA_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
