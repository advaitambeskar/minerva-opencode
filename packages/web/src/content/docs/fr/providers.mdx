...
Pour une configuration spécifique au projet ou persistante, utilisez `minerva.json` :
...
Fichier `minerva.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `minerva.json` :
...
4. Créez ou mettez à jour votre fichier `minerva.json` dans le répertoire de votre projet :
