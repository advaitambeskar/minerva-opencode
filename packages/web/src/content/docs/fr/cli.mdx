...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Mot de passe d'authentification de base (par défaut `MINERVA_SERVER_PASSWORD`)                    |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `MINERVA_SERVER_USERNAME` ou `opencode`) |
...
Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'OpenCode sans l'interface TUI. Définissez `MINERVA_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à OpenCode via une interface Web. Définissez `MINERVA_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
| ------------------------------------- | ------- | --------------------------------------------------------------------------------- |
| `MINERVA_AUTO_SHARE`                 | booléen | Partager automatiquement des sessions                                             |
| `MINERVA_GIT_BASH_PATH`              | chaîne  | Chemin vers l'exécutable Git Bash sur Windows                                     |
| `MINERVA_CONFIG`                     | chaîne  | Chemin d'accès au fichier de configuration                                        |
| `MINERVA_TUI_CONFIG`                 | chaîne  | Chemin d'accès au fichier de configuration TUI                                    |
| `MINERVA_CONFIG_DIR`                 | chaîne  | Chemin d'accès au répertoire de configuration                                     |
| `MINERVA_CONFIG_CONTENT`             | chaîne  | Contenu de configuration JSON en ligne                                            |
| `MINERVA_DISABLE_AUTOUPDATE`         | booléen | Désactiver les vérifications automatiques des mises à jour                        |
| `MINERVA_DISABLE_PRUNE`              | booléen | Désactiver le nettoyage des anciennes données                                     |
| `MINERVA_DISABLE_TERMINAL_TITLE`     | booléen | Désactiver les mises à jour automatiques des titres du terminal                   |
| `MINERVA_PERMISSION`                 | chaîne  | Configuration des autorisations JSON intégrées                                    |
| `MINERVA_DISABLE_DEFAULT_PLUGINS`    | booléen | Désactiver les plugins par défaut                                                 |
| `MINERVA_DISABLE_LSP_DOWNLOAD`       | booléen | Désactiver les téléchargements automatiques du serveur LSP                        |
| `MINERVA_ENABLE_EXPERIMENTAL_MODELS` | booléen | Activer les modèles expérimentaux                                                 |
| `MINERVA_DISABLE_AUTOCOMPACT`        | booléen | Désactiver le compactage automatique du contexte                                  |
| `MINERVA_DISABLE_CLAUDE_CODE`        | booléen | Désactiver la lecture de `.claude` (prompt + compétences)                         |
| `MINERVA_DISABLE_CLAUDE_CODE_PROMPT` | booléen | Désactiver la lecture `~/.claude/CLAUDE.md`                                       |
| `MINERVA_DISABLE_CLAUDE_CODE_SKILLS` | booléen | Désactiver le chargement de `.claude/skills`                                      |
| `MINERVA_DISABLE_MODELS_FETCH`       | booléen | Désactiver la récupération de modèles à partir de sources distantes               |
| `MINERVA_FAKE_VCS`                   | chaîne  | Faux fournisseur VCS à des fins de test                                           |
| `MINERVA_CLIENT`                     | chaîne  | Identifiant du client (par défaut `cli`)                                          |
| `MINERVA_ENABLE_EXA`                 | booléen | Activer les outils de recherche Web Exa                                           |
| `MINERVA_SERVER_PASSWORD`            | chaîne  | Activer l'authentification de base pour `serve`/`web`                             |
| `MINERVA_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `opencode`) |
| `MINERVA_MODELS_URL`                 | chaîne  | URL personnalisé pour récupérer la configuration des modèles                      |
...
| Variables                                       | Type    | Description                                                     |
| ----------------------------------------------- | ------- | --------------------------------------------------------------- |
| `MINERVA_EXPERIMENTAL`                         | booléen | Activer les fonctionnalités expérimentales du drapeau global    |
| `MINERVA_EXPERIMENTAL_ICON_DISCOVERY`          | booléen | Activer la découverte d'icônes                                  |
| `MINERVA_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | booléen | Désactiver la copie lors de la sélection dans TUI               |
| `MINERVA_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | numéro  | Délai d'expiration par défaut pour les commandes bash en ms     |
| `MINERVA_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | numéro  | Nombre maximum de jetons de sortie pour les réponses LLM        |
| `MINERVA_EXPERIMENTAL_FILEWATCHER`             | booléen | Activer l'observateur de fichiers pour l'ensemble du répertoire |
| `MINERVA_EXPERIMENTAL_OXFMT`                   | booléen | Activer le formateur oxfmt                                      |
| `MINERVA_EXPERIMENTAL_LSP_TOOL`                | booléen | Activer l'outil expérimental LSP                                |
| `MINERVA_EXPERIMENTAL_DISABLE_FILEWATCHER`     | booléen | Désactiver l'observateur de fichiers                            |
| `MINERVA_EXPERIMENTAL_EXA`                     | booléen | Activer les fonctionnalités Exa expérimentales                  |
| `MINERVA_EXPERIMENTAL_LSP_TY`                  | booléen | Activer TY LSP pour les fichiers python                         |
| `MINERVA_EXPERIMENTAL_PLAN_MODE`               | booléen | Activer le mode plan                                            |
| `MINERVA_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | booléen | Activer les tâches de sous-agents en arrière-plan               |
| `MINERVA_EXPERIMENTAL_EVENT_SYSTEM`            | booléen | Activer le système d'événements expérimental                    |
| `MINERVA_EXPERIMENTAL_NATIVE_LLM`              | booléen | Activer le chemin de requête LLM natif                          |
| `MINERVA_EXPERIMENTAL_PARALLEL`                | booléen | Activer l'exécution parallèle de la recherche web               |
| `MINERVA_EXPERIMENTAL_SCOUT`                   | booléen | Activer le sous-agent Scout                                     |
| `MINERVA_EXPERIMENTAL_WORKSPACES`              | booléen | Activer la prise en charge des espaces de travail               |
