...
- **macOS/Linux** : `~/.config/opencode/minerva.jsonc` (ou `~/.config/opencode/minerva.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/opencode/minerva.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\minerva.jsonc`
...
Si votre `minerva.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `MINERVA_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `MINERVA_PORT` (ou choisissez un port libre) et redémarrez.
