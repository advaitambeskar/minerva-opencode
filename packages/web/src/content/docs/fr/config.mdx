...
2. **Configuration globale** (`~/.config/opencode/minerva.json`) - préférences utilisateur
3. **Configuration personnalisée** (`MINERVA_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`minerva.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.opencode`** - agents, commandes, plugins
6. **Configuration en ligne** (`MINERVA_CONFIG_CONTENT` env var) - remplacements d'exécution
...
Placez votre configuration globale OpenCode dans `~/.config/opencode/minerva.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.
...
Ajoutez `minerva.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `MINERVA_CONFIG`.

```bash
export MINERVA_CONFIG=/path/to/my/custom-config.json
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `MINERVA_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.opencode`, et devrait suivre la même structure.

```bash
export MINERVA_CONFIG_DIR=/path/to/my/config-directory
opencode run "Hello world"
```
...
Le fichier de configuration a un schéma défini dans [**`opencode.ai/config.json`**](https://minerva.dev/config.json).
...
Utilisez `MINERVA_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `minerva.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
