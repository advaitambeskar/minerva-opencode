...
2. **全局配置**（`~/.config/opencode/minerva.json`）- 用户偏好
3. **自定义配置**（`MINERVA_CONFIG` 环境变量）- 自定义覆盖
4. **项目配置**（项目中的 `minerva.json`）- 项目特定设置
5. **`.opencode` 目录** - 代理、命令、插件
6. **内联配置**（`MINERVA_CONFIG_CONTENT` 环境变量）- 运行时覆盖
...
将全局 OpenCode 配置放在 `~/.config/opencode/minerva.json` 中。使用全局配置来设置用户级别的偏好，例如主题、提供商或快捷键。
...
在项目根目录中添加 `minerva.json`。项目配置在标准配置文件中具有最高优先级——它会覆盖全局配置和远程配置。
...
使用 `MINERVA_CONFIG` 环境变量指定自定义配置文件路径。
...
使用 `MINERVA_CONFIG_DIR` 环境变量指定自定义配置目录。该目录会像标准 `.opencode` 目录一样被搜索代理、命令、模式和插件，并且应遵循相同的结构。
...
配置文件具有在 [**`opencode.ai/config.json`**](https://minerva.dev/config.json) 中定义的 Schema。
