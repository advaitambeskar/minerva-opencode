...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `minerva.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `MINERVA_TUI_CONFIG` kullanın.
