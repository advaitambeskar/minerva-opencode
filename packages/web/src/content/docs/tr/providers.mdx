...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `minerva.json` kullanın:
...
`minerva.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`minerva.json` üzerinden özelleştirin:
...
4. Proje dizininizde `minerva.json` dosyanızı oluşturun veya güncelleyin:
