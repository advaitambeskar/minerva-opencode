...
2. **Genel yapılandırma** (`~/.config/opencode/minerva.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`MINERVA_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `minerva.json`) - projeye özgü ayarlar
5. **`.opencode` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`MINERVA_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
Global opencode yapılandırmanızı `~/.config/opencode/minerva.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `minerva.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.
...
`MINERVA_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`MINERVA_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
...
Yapılandırma dosyası [**`opencode.ai/config.json`**](https://minerva.dev/config.json)'da tanımlanan bir şemaya sahiptir.
