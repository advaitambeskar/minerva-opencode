...
- **macOS/Linux**: `~/.config/opencode/minerva.jsonc` (veya `~/.config/opencode/minerva.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/opencode/minerva.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\minerva.jsonc` yapistirin
...
`minerva.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `MINERVA_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `MINERVA_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
