...
import { type Session } from "@minerva-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("minerva://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("minerva://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("minerva://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("minerva://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("minerva://open-project")).toBeUndefined()
    expect(parseDeepLink("minerva://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "minerva://open-project?directory=/a",
      "minerva://other?directory=/b",
      "minerva://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("minerva://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("minerva://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("minerva://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("minerva://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "minerva://new-session?directory=/a",
      "minerva://open-project?directory=/b",
      "minerva://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __MINERVA__: {
        deepLinks: ["minerva://open-project?directory=/a"],
      },
    } as unknown as Window & { __MINERVA__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["minerva://open-project?directory=/a"])
